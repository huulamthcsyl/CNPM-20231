...
export default function ResidenceFeeDetailPage() {
  const navigate = useNavigate();
  const residenceFeeId = useParams().id;
  ...
  useEffect(() => {
    setTotalFeeAmount(0);
    residenceReceipts.forEach((data) =>
      ...
      <Grid container spacing={2} style={{ padding: "50px" }}>
        ...
        <Grid item xs={12}>
          <span style={{ fontSize: "22px" }}>
            <b>Số người đã đóng: </b>
          </span>
          <span style={{ fontSize: "22px" }}>{fee.paidQuantity}</span>
        </Grid>
        <Grid item xs={12}>
